...
async def research_evidence(settings: Settings) -> tuple[list[dict[str, Any]], str]:
    ...
    try:
        response = await client.responses.parse(
            ...
            instructions=(
                "Пиши все описания по-русски, без английских терминов. "
...
async def advise_scenario(
    ...
    try:
        ...
        response = await client.responses.parse(
            ...
            instructions=(
                "Пиши все пояснения по-русски. Вместо evidence, output, outcome, score, "
                "baseline и SLA используй источники, выполненные работы, результат, индекс, "
                "исходное состояние и сроки обслуживания. Технические ключи JSON сохраняй. "
